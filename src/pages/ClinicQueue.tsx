...
export default function ClinicQueue({ board = false }: { board?: boolean }) {
  const tickets = useStore(queueRepository); const { currentUser } = useAppState(); const { message } = App.useApp();
  ...
  const act = (fn: () => QueueTicket, success: string, voice = false) => { try { const t = fn(); if (voice) announce(t); message.success(success); } catch(e) { message.error(e instanceof Error ? e.message : String(e)); } };
  return <div style={{ display:'flex', flexDirection:'column', gap:16 }}><div><Text type="secondary">VẬN HÀNH PHÒNG KHÁM</Text><Title level={3}>Hàng đợi khám bệnh</Title></div><Space><Button type="primary" icon={<BellRing size={16}/>} onClick={()=>act(()=>queueService.callNext('Khoa Da liễu',currentUser.id),'Đã gọi bệnh nhân tiếp theo.',true)}>Gọi số tiếp theo</Button><Button href="/display/queue" target="_blank">Mở bảng hiển thị</Button></Space>
    <Card><Table rowKey="id" dataSource={tickets} locale={{emptyText:<Empty description="Chưa có bệnh nhân check-in"/>}} columns={[
